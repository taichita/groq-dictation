...
ホットキー（既定 `右Ctrl + Alt`）を押す → 話す → もう一度押す → 文字起こし結果が今フォーカスしている入力欄に自動で貼り付けられる。録音中は **画面右下に赤い丸**が点灯します（**音は鳴りません**）。PC起動時に自動で常駐するので、いつでもホットキーひとつで使えます。**長く話し続けてもOK**で、長い録音は自動で「間」ごとに分割して送るため、途中で切れません。
...
| `MAX_RECORD_SEC` | **0** | 録音の自動停止（**0=無制限**）。秒数を入れるとその秒で自動停止 |
| `CHUNK_SEC` | 180 | 長い録音をこの秒数ごとに「間」で**自動分割**して送信（413回避・通常は変更不要） |
...
| `413 Payload Too Large` | 1回の送信が大きすぎ | 通常は自動分割で回避。もし出るなら `CHUNK_SEC` を小さく（例 120） |
